    ...
    ui(new Ui::Server)
{
    ...
    connect(severe, &QTcpServer::newConnection, this, &Server::newConnectSlot);     //有新连接时传入信号
}
...
void Server::enumAllIp()    //初始化本机所有可用的ipv4的地址并添加地址到列表框中
{
...
void Server::newConnectSlot()    //新的客户端连接
{
    connectSound->play();
    ...
    connect(clintSock, &QTcpSocket::readyRead, this, &Server::readMessage);  //准备接受信息
    connect(clintSock, &QTcpSocket::disconnected, this, &Server::disConnectSlot);   //客户端的断开连接信息

}

void Server::readMessage()      //读取客户端信息及给其他客户端发送信息
{
...
void Server::disConnectSlot()       //客户端断开连接时服务端显示断开信息
{
    connectSound->play();
...
void Server::on_listenBtn_clicked()
{
...
void Server::on_back_btn_clicked()
{
    this->hide();
    this->backToSelect();
}
